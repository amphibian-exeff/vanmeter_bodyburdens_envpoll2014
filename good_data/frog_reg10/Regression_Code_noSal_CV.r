...
####load data############
frog_root<-"D:\\Dropbox\\Robin_data_test\\frog_reg10\\" #PC path
setwd(frog_root)
...
for (k in 1:5){
  ...
  R2_logBCF=array(dim=c(total_ita-1,11))
  colnames(R2_logBCF)=c("R2", "Adj_R2", "AIC", "BIC", "MSE_T", "LL_T", "LL_R", "df_T", "df_R", "N_reg", "N_var")
...
CV_RMSE_Final=(colSums(CV_res_all^2)/131)^0.5
NCV_RMSE_Final=(colSums(NCV_res_all^2)/131)^0.5
...
Final_LL=data.frame(R2_logBCF[,10], R2_logBCF[,11], CV_LL_Final, NCV_MC_LL_Final)
names(Final_LL)=c('N_reg','N_var','LL_T', name_LL)


write.table(Final_RMSE, file = "Final_RMSE.xls", sep="\t", row.names = F)
write.table(Final_LL, file = "Final_LL.xls", sep="\t", row.names = F)


###############Compare###################################################
loglike_test((lm_temp$fit-mlrfrog$logBCF), 0, summary(lm_temp)$sigma)
