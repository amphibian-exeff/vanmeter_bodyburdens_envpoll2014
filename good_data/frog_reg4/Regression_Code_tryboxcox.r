...
####load data############
frog_root<-"D:\\Dropbox\\Robin_data_test\\frog_reg4\\" #PC path
setwd(frog_root)
...
###Regression BCF################
#######################################
mlrfrog_BCF_test=subset(mlrfrog,select=c(BCF,Species,HabFac,logKow,logkoc,Hlaw,VapPrs_mPa))
...
mlrfrog_BCF=subset(mlrfrog_BCF_test,select=-c(BCF,logKow,Hlaw,VapPrs_mPa)) 
...
R2_BCF=array(dim=c(total_ita-1,4))
colnames(R2_BCF)=c("R2", "Adj_R2", "AIC", "BIC")

for (i in 1:(total_ita-1)){
  vect=as.matrix(detail_select)[i,]
  lm_temp=lm(as.formula(paste("mlrfrog$BCF", paste("mlrfrog_BCF$",regressors[vect], collapse=" + "), sep="~")))
  name_BCF=paste("sum_BCF", i, sep="")
  assign(name_BCF, (lm_temp))
  R2_BCF[i,1]=summary(lm_temp)$r.squared
  R2_BCF[i,2]=summary(lm_temp)$adj.r.squared
  R2_BCF[i,3]=AIC(lm_temp)
  R2_BCF[i,4]=BIC(lm_temp)
}

####The highest R2(1), adjusted R2(2), lowest AIC(3) and BIC(4)
col_best_BCF=c(apply(R2_BCF,2,max)[1:2],apply(R2_BCF,2,min)[3:4])
final_BCF=c(which(R2_BCF[,1]==col_best_BCF[1]),which(R2_BCF[,2]==col_best_BCF[2]),
                     which(R2_BCF[,3]==col_best_BCF[3]),which(R2_BCF[,4]==col_best_BCF[4]))
mfc_BCF=final_BCF[4]#find the most common combination ID
best_rg_BCF=paste("sum_BCF", mfc_BCF, sep="")   #recommended regression
coef_BCF=summary(eval(parse(text=best_rg_BCF)))$coefficients 
...
#######################################
mlrfrog_tc_test=subset(mlrfrog,select=c(logTissueConc,Species,HabFac,logKow,logkoc,Hlaw,VapPrs_mPa,app_rate_g_cm2,bodyweight2,SA_cm2_2))
cor_matrix_tc_test=cor(subset(mlrfrog_tc_test,select=-c(HabFac,Species)))

#####Based on the correlation matrix, I can only keep a.Species, b.HabFac, c.logkoc, d.app_rate_g_cm2, e.bodyweight2, f.SA_cm2_2
mlrfrog_tc=subset(mlrfrog_tc_test,select=-c(logTissueConc,logKow,Hlaw,VapPrs_mPa)) 
...
#######################################
mlrfrog_sc_test=subset(mlrfrog,select=c(logSoilConc,app_rate_g_cm2,logKow,logkoc,Hlaw,VapPrs_mPa))
...
#######################################
mlrfrog_af_test=subset(mlrfrog,select=c(logAppFactor,Species,HabFac,logKow,logkoc,Hlaw,VapPrs_mPa))
...
mlrfrog_af=subset(mlrfrog_af_test,select=-c(logAppFactor,logKow,Hlaw,VapPrs_mPa)) 
detail_select=expand.grid(rep(list(c(TRUE,FALSE)),3))
detail_select=detail_select[-which(detail_select$Var1==TRUE & detail_select$Var2==TRUE),]
row.names(detail_select)=NULL
