...
####load data############
frog_root<-"D:\\Dropbox\\Robin_data_test\\frog_reg5\\" #PC path
setwd(frog_root)
...
mlrfrog = subset(raw_data_t2, select=c(Species, app_rate_g_cm2, AppFactor, TissueConc, SoilConc, logKow, BCF, VapPrs_mPa, Koc_Lab,
                                           HabFac, Hlaw, bodyweight2, SA_cm2_2, molmass_gmol)) 
...
###Regression BCF################
#######################################
mlrfrog_BCF_test=subset(mlrfrog,select=c(logBCF,Species,HabFac,logKow,logkoc,Hlaw,VapPrs_mPa, molmass_gmol))
...
detail_select=expand.grid(rep(list(c(TRUE,FALSE)),4))
...
#   lm_BC_temp=as.formula(paste("mlrfrog$logBCF", paste("mlrfrog_BCF$",regressors[vect], collapse=" + "), sep="~"))
#   bc_temp=boxcox(lm_BC_temp)
#   bc_temp_x=bc_temp$x[which(bc_temp$y==max(bc_temp$y))]
#   mlrfrog$logBCF_temp=bcPower(mlrfrog$logBCF, bc_temp_x)
...
mfc_logBCF=final_logBCF[2]#find the most common combination ID
...
#######################################
mlrfrog_tc_test=subset(mlrfrog,select=c(logTissueConc,Species,HabFac,logKow,logkoc,Hlaw,VapPrs_mPa,app_rate_g_cm2,bodyweight2,SA_cm2_2, molmass_gmol))
...
detail_select=expand.grid(rep(list(c(TRUE,FALSE)),7))
detail_select=detail_select[-which(detail_select$Var1==TRUE & detail_select$Var2==TRUE),]
row.names(detail_select)=NULL
...
mfc_logTissueConc=final_logTissueConc[2]#find the most common combination ID
...
#######################################
mlrfrog_sc_test=subset(mlrfrog,select=c(logSoilConc,app_rate_g_cm2,logKow,logkoc,Hlaw,VapPrs_mPa,molmass_gmol))
...
mfc_logSoilConc=final_logSoilConc[2]#find the most common combination ID
...
#######################################
mlrfrog_af_test=subset(mlrfrog,select=c(logAppFactor,Species,HabFac,logKow,logkoc,Hlaw,VapPrs_mPa,molmass_gmol))
...
mfc_logAppFactor=final_logAppFactor[2]
